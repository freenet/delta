...
#
# The key is read from ~/.config/delta/web-container-keys.toml — the same key
# that signs Delta's web container, following the precedent Ian set for River on
# 2026-08-18. It is piped straight into the signer and never lands in a
# variable, a temp file, or argv.
#
# Delta has no other publisher identity: its data model is per-SITE owner keys,
# so there is no pre-existing "Delta author" key. See pointer-records.toml.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
TOML_PATH="pointer-records.toml"
KEY_FILE="${DELTA_POINTER_KEY_FILE:-$HOME/.config/delta/web-container-keys.toml}"
...
# whole trust anchor for every integrator resolving a Delta pointer.
#
