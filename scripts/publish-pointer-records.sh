#!/bin/bash
# Publish Delta's signed pointer records to the Freenet network.
#
...
WEB_CONTAINER_KEY="EqJ5YpEEV3XLqEvKWLQHFhGAac2qXzSUoE6k2zbdnXBr"
...
RUNS="$(gh run list --repo freenet/delta --commit "$HEAD_SHA" --json conclusion,status,name 2>/dev/null || echo "")"
...
if [ "$BAD" != "0" ]; then
    gh run list --repo freenet/delta --commit "$HEAD_SHA" --limit 20 | sed 's/^/    /'
...
(cargo make publish-delta), or re-sign against the live bytes."
        fi
